...
func (r *RouterDCSelector) InitializeModelEmbeddings(modelDescriptions map[string]string) error {
	...
	for model, description := range modelDescriptions {
		...
		if err != nil {
			klog.Warningf("[RouterDC] Failed to embed model %s description: %v", model, err)
			continue
		}
		r.modelEmbeddings[model] = embedding
	}

	klog.V(4).InfoS("router_dc_embeddings_initialized", "component", "selection", "details", map[string]interface{}{
...
func (r *RouterDCSelector) InitializeFromConfig(modelConfig map[string]ModelParams) error {
	if r.embeddingFunc == nil {
		klog.Warningf("[RouterDC] No embedding function set, cannot initialize model embeddings")
		return nil
	}
	...
	for model, params := range modelConfig {
		...
		if err != nil {
			klog.Warningf("[RouterDC] Failed to embed model %s: %v", model, err)
			continue
		}

		r.modelEmbeddings[model] = embedding
		modelsWithDescriptions++
		klog.V(4).InfoS("router_dc_model_embedding_initialized", "component", "selection", "model", model)
	}

	klog.V(4).InfoS("router_dc_config_embeddings_initialized",
		"component", "selection",
...
func (r *RouterDCSelector) buildDescriptionText(model string, params ModelParams) string {
	var parts []string
...
func (r *RouterDCSelector) ValidateConfig(modelConfig map[string]ModelParams) error {
...
func (r *RouterDCSelector) Select(ctx context.Context, selCtx *SelectionContext) (*SelectionResult, error) {
	...
	var bestModel *ModelRef
	var bestScore float64

	r.embeddingMu.RLock()
	defer r.embeddingMu.RUnlock()

	klog.Infof("[RouterDC] Evaluating %d candidates by embedding similarity:",
	...
	for i := range selCtx.CandidateModels {
		...
		if !exists {
			...
			klog.Infof("[RouterDC]   %s: similarity=%.4f (no embedding, using min)", model.Model, r.config.MinSimilarity)
			continue
		}
		...
		klog.Infof("[RouterDC]   %s: similarity=%.4f", model.Model, similarity)
	...
	klog.Infof("[RouterDC] Selected model %s (similarity=%.4f, confidence=%.2f)",
...
func (r *RouterDCSelector) UpdateFeedback(ctx context.Context, feedback *Feedback) error {
	...
	klog.V(4).Infof("[RouterDC] Updated affinity for query hash %s: winner=%s (+0.1)",
...
func (r *RouterDCSelector) fallbackSelection(selCtx *SelectionContext, reason string) (*SelectionResult, error) {
	...
	klog.Warningf("[RouterDC] Fallback selection: %s, using first candidate %s", reason, firstModel.Model)
