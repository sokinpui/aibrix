...
func (s *StaticSelector) InitializeFromConfig(categories []Category) {
	s.scoresMu.Lock()
...
func (s *StaticSelector) UpdateFeedback(ctx context.Context, feedback *Feedback) error {
	...
	klog.V(4).Infof("[StaticSelector] Ignoring feedback (static selector does not learn)")
	return nil
}

// getModelNames extracts model names from ModelRef slice
func getModelNames(models []ModelRef) []string {
